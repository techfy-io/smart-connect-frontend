  ...
  const handleSubmitExchangeModal = async (values) => {
    ...
    try {
      const response = await axios.post(
        `${process.env.REACT_APP_BASE_API_URL}/exchange/`,
        formData
      );
    ...
    if (imageToSend) {
      fetch(imageToSend)
        ...
        .then((blob) => {
          ...
          axios
            .put(
              `${process.env.REACT_APP_BASE_API_URL}/user/image/${userData.id}/`,
              formData ? formData : selectedImage,
              {
                headers: {
    ...
    >
      {pageloading ? (
          ...
          <div className={`ecard-sidebar ${sidebarOpen ? "open" : ""}`}>
            ...
            {checkLoginUser == null ? (
              <div className="profile-login-btn">
                <Tooltip
                  title={t(
                    "Are you the owner of the file? Click here to login?"
                  )}
                >
                  <Button className="profile-action-login-btn" type="primary">
            ...
            <p className="qr-code-para">
              {t("Show QRCode to share your profile")}
            </p>
            <div className="shope-link">
              {t("Shope Link")}:{" "}
              <a href="https://smartconnect.cards/">
                {" "}
                https://smartconnect.cards
              </a>
            </div>
          </div>
        </>
        ...
        <div style={{ margin: "auto" }}>
          <Empty
            size="Large"
            description={t("The User is no longer available")}
          />
        </div>
      )}
      {editingCover && (
        ...
        >
          ...
          <div style={{ display: "flex" }}>
            ...
            />
            <Button onClick={handleImageUpload} style={{ marginRight: "5px" }}>
